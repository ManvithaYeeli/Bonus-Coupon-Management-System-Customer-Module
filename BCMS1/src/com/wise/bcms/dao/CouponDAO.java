...
public class CouponDAO {
    ...
    public Coupon get(int couponId) {
		...
		final String QUERY = "select c.coupon_id, c.validity, c.denomination_id, d.amount from coupons c, denomination d where d.denomination_id = c.denomination_id and c.coupon_id = ?";
		try(Connection connection = DBUtility.getConnection()){
			preparedStatement = connection.prepareStatement(QUERY);
    ...
    public List<Coupon> get() {
    	final String QUERY = "select c.coupon_id, c.validity, d.denomination_id, d.amount from coupons c, denomination d where d.denomination_id = c.denomination_id";
    	ResultSet resultSet = null;
    ...
    public List<Coupon> getCoupons(int denominationId) {
    	final String QUERY = "SELECT coupons.coupon_id, coupons.denomination_id, coupons.validity,denomination.amount, FROM coupons  INNER JOIN denomination ON denomination.denomination_id=? and coupons.denomination_id=denomination.denomination_id and coupons.activate = ?";
    	ResultSet resultSet = null;
    	List<Coupon> list = new ArrayList<Coupon>();
  ...
  /*  public List<CustomerCoupon> getCustomerCoupons(int customerId) {
    final String QUERY = "select cc.coupon_id, cc.validity_date, cc.month, cc.year, d.amount from customer_coupons cc, coupon c, denomination d where c.coupon_id = cc.coupon_id and d.denomination_id = c.denomination_id and cc.customer_id = ?";
    	ResultSet resultSet = null;
    	...
    	try(Connection connection = DBUtility.getConnection()) {
			...
			while(resultSet.next()) {
				...
				customerCoupon.setValidDate(resultSet.getDate(2));
