...
public class OrderLineDAO {
	...
	public int add(OrderLine orderLine) {
		int status = 0;
		final String QUERY = "insert into orders_line(qty,product_id,orders_id,price) values(?,?,?,?)";
	...
	public List<OrderLine> get(int customerId,int orderId) {
		final String QUERY = "select ol.orders_id,  "
				+ "p.product_name, p.picture, ol.qty, ol.price from orders o, orders_line ol, "
				+ "products p where o.customer_id = ? and  ol.orders_id = o.orders_id and "
				+ "ol.product_id=p.product_id and ol.orders_id=? ";
