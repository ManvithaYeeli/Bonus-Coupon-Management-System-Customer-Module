...
public class CustomerDAO {
	public int add(Customer customer) {		
		int status = 0;
		final String QUERY = "insert into customer(customer_id,customer_name,customer_surname,dob,gender,password,email,phone_no,company_id,house_no,location,city,state,country,pincode) values(?, ?, ?, ?, ?, ?, ?, ? ,? ,?, ?, ?, ?, ?, ?)";
		try(Connection connection = DBUtility.getConnection()){
			PreparedStatement preparedStatement = connection.prepareStatement(QUERY);
			preparedStatement.setInt(1, customer.getId());
			preparedStatement.setString(2, customer.getName());
			preparedStatement.setString(3, customer.getSurname());
			preparedStatement.setDate(4, DBUtility.convertFromUTILDateToSQLDate(customer.getDob()));
			preparedStatement.setString(5, customer.getGender());
			preparedStatement.setString(7, customer.getEmail());
			preparedStatement.setString(6, customer.getPassword());
			preparedStatement.setString(8, customer.getPhno());
			preparedStatement.setInt(9, customer.getCompany().getId());
			preparedStatement.setString(10, customer.getHouseno());
	...
	public Customer get(int customerId){	
		...
		try (Connection connection = DBUtility.getConnection()) {
			...
			if(resultSet.next()) {
				...
				customer.setDob(DBUtility.convertFromSQLDateToUTILDate(resultSet.getDate(4)));
				...
				customer.setPhno(resultSet.getString(8));
				customer.setHouseno(resultSet.getString(10));
