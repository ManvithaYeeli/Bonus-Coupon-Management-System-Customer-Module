...
public class CouponReqDAO {
	...
	public List<CouponReq> get(int companyId) {
		...
		ResultSet resultSet = null;
		PreparedStatement preparedStatement = null;
		final String QUERY = "select cr.customer_id, cr.amount, cr.month, cr.year from coupon_reqs cr, customer c where c.customer_id = cr.customer_id and c.company_id = ?";
		try (Connection connection = DBUtility.getConnection()) {
			preparedStatement = connection.prepareStatement(QUERY);
